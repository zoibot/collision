...
double object_manager::impulse_scale;
...
void object_manager::handle_collision(vec2 collidea, object *a, object *b) {
    ...
    if(!twoa || !twob) {
        ...
        debug_layer::lines.push_back(colored_line(collisionpt, collisionpt+vab, color(0,0,1)));
        ...
        //this adds momentum somehow?
        impulse_scale = vab.scale(-(1 + e)).dot(collidea);
        double impulse_scale_denom = (collidea.dot(collidea)*(1/a->mass+1/b->mass)
        ...
        debug_layer::lines.push_back(colored_line(collisionpt - impulse, color(1,0.9,0.9), collisionpt + impulse, color(1,0,0)));
